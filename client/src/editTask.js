...
const EditTask = ({ open, onClose, onEditTask, taskToEdit }) => {
  ...
  useEffect(() => {
    if (taskToEdit) {
        ...
        const priorityMap = {
          ...
          setPriority(priorityMap[taskToEdit.priority] || '');
      }
  ...
  const handleEditTask = async () => {
    ...
    if (title.length >= 2 && description.length >= 2 && dueDate >= new Date()) {
      try {
        ...
        if (!taskToEdit) {
            console.error('Task to edit is not defined');
            return;
          }
          console.log('Task ID to edit:', taskToEdit._id);
        ...
        const response = await fetch(`http://localhost:8080/api/tasks/editTasks/${taskToEdit._id}`, {
          method: 'PUT',
          headers: {
  ...
  const handleInputChange = (e) => {
    ...
    // Example: setTitle(e.target.value);
  };
